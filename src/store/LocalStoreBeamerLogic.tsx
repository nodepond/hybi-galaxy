...
import { useLocalBeamerStore } from "./LocalBeamerStore"
...
export const LocalStoreLogic = () => {
  ...
  const id = useLocalBeamerStore(store => store.id)
  const pos = useLocalBeamerStore(store => store.pos)
  const room = useLocalBeamerStore(store => store.room)
  const setLocalTracks = useLocalBeamerStore(store => store.setLocalTracks)
  const setMyID = useLocalBeamerStore(store => store.setMyID)
